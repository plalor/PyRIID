...
class TransformerPlus(PyRIIDModel):
    """Transformer classifier."""
    def __init__(self, activation=None, loss=None, optimizer=None, metrics=None,
                 embed_mode=None, final_activation=None, embed_dim=None, num_heads=None, ff_dim=None,
    ...
    def fit(self, training_ss: SampleSet, validation_ss: SampleSet, batch_size: int = 200,
        ...
        if not self.model:
            ...
            # Patch embedding: slide a window of size `patch_size` (with step `stride`) over the 1D spectrum
            # and project each patch into an `embed_dim`-dimensional token in one fused Conv1D operation
